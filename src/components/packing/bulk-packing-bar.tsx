...
export function BulkPackingBar({ items, onComplete }: BulkPackingBarProps) {
  ...
  // Group item ids by category for "check all in category" buttons
  ...
  const activeCategories = Array.from(categorizedIds.entries())
    .map(([value, ids]) => {
      const meta = PACKING_CATEGORIES.find((c) => c.value === value);
...
// Export type for parent integration
export type { BulkPackingBarProps };
